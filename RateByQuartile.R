...
# Summarize table to 1 row per quartile
quartile_summary <- quartile_flex_table %>%
  group_by(Quartile) %>%
  summarise(
    `Avg Coefficient` = round(mean(`Regression Coefficient`, na.rm = TRUE), 2),
    `Avg Effect of 10% Increase` = round(mean(`Effect of 10% Increase`, na.rm = TRUE), 2),
    `Significant Measures` = n(),
    .groups = "drop"
  )

# Display as a flextable
flextable(quartile_summary) %>%
  set_caption("Summary by Quartile: Average Coefficient and Number of Significant Results") %>%
  fontsize(size = 10, part = "all") %>%
  autofit() %>%
  align(align = "center", part = "all")

write.csv(quartile_summary, "rate_by_quartile.csv", row.names = FALSE)
